...
class Validation {
  ...
  */
  constructor(form) {
    ...
    this.boundFormInputHandler = debounce(e => this.validation(e), 100, true);
    this.boundFormChangeHandler = debounce(e => this.validation(e), 100, true);
  ...
  registerHandlers() {
    this.init();
    this.form.addEventListener(EVENT_SUBMIT, e => this.validation(e));
  }
  ...
  validation(event) {
    ...
    for (const value of this.storeErrors.values()) {
      if (value !== '') {
